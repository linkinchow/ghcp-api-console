...
- [ ] 长停顿后旧进程恢复、SSO/SCIM/seat其它副作用边界、网络GET停滞/服务重启及其它凭据/取消排列仍待验证；SIGKILL子场景不能声称覆盖活着的旧进程迟到恢复。
...
本机原因/状态诊断的实现、本地回归、新镜像选择性运行验收及运行日志/RestartCount独立证据核对均已通过，本轮限定交付全部收口，不重复列为待执行或待核对。冻结v4四项验收及本轮有限进程用例继续有效，冻结生产代码未改；当前没有证据表明MySQL v4存在“单次失权后整个实例永久pool_owner_unavailable”的同型故障。全局告警、其余进程排列与客户SQLite恢复仍是范围外的独立事项，不能把本机诊断交付写成全部503问题关闭，本轮不新增目标。
