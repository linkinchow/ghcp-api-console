...
更新：2026-09-15。当前工作分支`ghcp-user-pool-observability`，基于冻结v4提交`2bc12b363e62923ca6c1db0185e42f9ed5c78bf9`（`2bc12b3`）。`ghcp-user-pool-mysql`及已验收v4镜像保持原样，冻结提交已commit、未push。后续分支已实现本机诊断增量，其本地回归和新镜像选择性运行验收已通过；结果单独记录，不沿用冻结v4的验收结论，也不是生产部署授权。
...
- [x] **外部操作已成功、进度未写回时精确切换owner——双驱动确定性组件契约完成。** SSO/SCIM/seat/Login各两屏障＋三种不可见结果，SQLite11＋MySQL11全部通过。可确认步骤GET恢复，不明SSO创建停住人工核对；旧owner实际SQL写入被拒绝。[专项报告](user-pool-owner-side-effects.md)。
  ...
  - [ ] SSO/SCIM/seat其余副作用边界、长停顿后旧进程恢复及传输中断排列仍待验证；SIGKILL不等于旧进程恢复迟到，组件强制owner到期不替代OS故障。
- [x] **目录查询压力＋部分取消＋同时推理——真实HTTP/双驱动组件层完成。** SQLite5＋MySQL2全部通过，catalog hold、模型刷新取消隔离、不续租、响应大小/时间界限均检查。
  ...
  - [ ] 全部消费者取消、count_tokens及更广压力排列仍未覆盖。
- [x] **旧凭据成功/401交错及替换/ABA——双驱动组件层完成。** 两种替换×两种返回顺序×两驱动，8个实际子场景通过；旧结果不破坏新凭据、不续租、不重放，重新warmup后才可用。
  ...
  - [ ] 非ABA替换、401-first等其余进程排列及进程退出时组合仍待补。
- [x] **Login轮询连续失败、重试耗尽后恢复——双驱动组件契约完成。** 最终SQLite3＋MySQL3通过；明确终态才释放槽位，running/cancelled/404/关联不符保留，failed/disabled不被自动重试。首次测试时序断言失败保留并修正。
  - [ ] 真实网络GET停滞至超时、独立Login/Proxy进程重启和真实退避耗时仍未验证；组件层503/传输拒绝、受控观察阻塞及本次worker超龄任务接管均不替代它们。
...
- [ ] 客户SQLite永久失权后的恢复机制单独评估和交付，尚未实现；客户当前报错不能视为MySQL v4相同缺陷已复现。
...
冻结v4四项最终回归、有限进程用例及observability增量的本地回归/选择性运行验收均已通过，各自范围保持分开。本轮功能、限定验收及运行日志/RestartCount独立证据核对全部收口，不重复列为待运行或待核对。其余进程排列、容量和全局告警仍属范围外的独立事项；客户仍运行SQLite，其当前故障取证/受控恢复单独处理。第2类人工处置和第4类生产HA/真实上游仍是独立上线门槛，本轮不新增目标。

冻结提交`2bc12b3`已提交、未push，其生产代码和已验收镜像不变。后续生产诊断增量、进程夹具与报告位于`ghcp-user-pool-observability`，不把新增代码称为冻结v4已验收内容；本次新镜像构建后未改生产源码。
